...
function drawTextInBubble(
  ...
  // 文字在气泡内垂直居中（扣掉上下各 bubblePadding 的 padding）
  const totalTextHeight = lines.length * lineHeight;
  const availableH = bubbleHeight - bubblePadding * 2;
  const textStartY = y + bubblePadding / 2 + (availableH - totalTextHeight) / 2;

  for (let i = 0; i < lines.length; i++) {
    const lineCenterY = textStartY + i * lineHeight;
    let curX = x;
...
function drawRedPacket(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, rp: Message['redPacket'], isOpened: boolean, _PAD: number, scale: number = 1) {
  ...
  ctx.fillText(statusText, x + w / 2, y + bodyH + footerH / 2);
}
...
function renderFrame(
  ...
  for (const layout of layouts) {
    ...
    if (msg.type === 'redpacket') {
      ...
      drawRedPacket(ctx, actualBubbleX, actualBubbleY + quoteH, layout.bubbleWidth, layout.bubbleHeight - quoteH, msg.redPacket, msg.redPacket?.isOpened || false, bubblePadding, scale);
    } else if (msg.type === 'transfer') {
      drawBubble(ctx, actualBubbleX, actualBubbleY, layout.bubbleWidth, layout.bubbleHeight, bubbleRadius, bubbleBg);
      const quoteH = drawQuote(ctx, actualBubbleX + bubblePadding, actualBubbleY + bubblePadding, layout.bubbleWidth - bubblePadding * 2, msg.quote, fontSize, scale);
      drawTransfer(ctx, actualBubbleX, actualBubbleY + quoteH, layout.bubbleWidth, layout.bubbleHeight - quoteH, msg.transfer, msg.transfer?.isReceived || false, bubblePadding, scale);
    } else if (msg.type === 'voice') {
      drawBubble(ctx, actualBubbleX, actualBubbleY, layout.bubbleWidth, layout.bubbleHeight, bubbleRadius, bubbleBg);
      const quoteH = drawQuote(ctx, actualBubbleX + bubblePadding, actualBubbleY + bubblePadding, layout.bubbleWidth - bubblePadding * 2, msg.quote, fontSize, scale);
      drawVoice(ctx, actualBubbleX, actualBubbleY + quoteH, layout.bubbleWidth, layout.bubbleHeight - quoteH, msg.voice, bubbleBg, bubblePadding, fontSize, lineHeight, scale, darkMode);
    } else if (msg.type === 'image') {
      drawBubble(ctx, actualBubbleX, actualBubbleY, layout.bubbleWidth, layout.bubbleHeight, bubbleRadius, bubbleBg);
      const quoteH = drawQuote(ctx, actualBubbleX + bubblePadding, actualBubbleY + bubblePadding, layout.bubbleWidth - bubblePadding * 2, msg.quote, fontSize, scale);
      drawImage(ctx, actualBubbleX, actualBubbleY + quoteH, layout.bubbleWidth, layout.bubbleHeight - quoteH, msg.image, imageCache, scale);
    } else if (msg.type === 'file') {
      drawBubble(ctx, actualBubbleX, actualBubbleY, layout.bubbleWidth, layout.bubbleHeight, bubbleRadius, bubbleBg);
      const quoteH = drawQuote(ctx, actualBubbleX + bubblePadding, actualBubbleY + bubblePadding, layout.bubbleWidth - bubblePadding * 2, msg.quote, fontSize, scale);
      drawFile(ctx, actualBubbleX, actualBubbleY + quoteH, layout.bubbleWidth, layout.bubbleHeight - quoteH, msg.file, bubblePadding, scale);
    } else if (text) {
      drawBubble(ctx, actualBubbleX, actualBubbleY, layout.bubbleWidth, layout.bubbleHeight, bubbleRadius, bubbleBg);
      const quoteH = msg.quote
        ? drawQuote(ctx, actualBubbleX + bubblePadding, actualBubbleY + bubblePadding, layout.bubbleWidth - bubblePadding * 2, msg.quote, fontSize, scale)
        : 0;
      const textStartY = actualBubbleY + bubblePadding + quoteH;
      // 有引用时直接用剩余高度（引用已含自己的padding），无引用时用完整高度
      const textAreaHeight = quoteH > 0
        ? layout.bubbleHeight - quoteH
        : layout.bubbleHeight;
      drawTextInBubble(ctx, text, actualBubbleX + bubblePadding, textStartY, layout.bubbleWidth - bubblePadding * 2, fontSize, bubbleColor, lineHeight, textAreaHeight, quoteH > 0 ? bubblePadding : 0);
    }
  }

  // Restore clip and draw header/status bar on top
  ctx.restore();
...
export class TypingVideoExporter {
  ...
  async export(
    ...
    const imageUrls = Array.from(new Set(messages
    ...
    await Promise.all([...avatarPromises, ...imagePromises]);
    ...
    for (let t = 0; t <= totalDuration; t += timeStep) {
      ...
      frameBuffer.push(new Uint8Array(arrayBuffer));
      frameIndex++;
    ...
    console.log('[TypingVideoExporter] Executing FFmpeg', {
      fps,
      frameIndex,
      args: [
    ...
    try {
      ...
      console.log('[TypingVideoExporter] FFmpeg execution completed successfully');
    } catch (err) {
      console.error('[TypingVideoExporter] FFmpeg exec error:', err);
      throw new Error('视频合成失败: ' + (err as Error).message);
    }

    onProgress?.(95);

    const data = await ffmpeg.readFile('output.mp4');
    console.log('[TypingVideoExporter] Read output.mp4 size=', data.length);
