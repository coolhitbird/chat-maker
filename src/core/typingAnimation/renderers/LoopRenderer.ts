...
export class LoopTypingRenderer implements TypingRenderer {
  ...
  private calculateLayout(
    ...
    for (const msg of messages) {
      ...
      if (sequences.has(msg.id) && !config.fastMode) {
        const result = getVisibleContentAtTime(sequences.get(msg.id)!, elapsedTime);
    ...
    for (let t = 0; t <= totalDuration; t += frameInterval) {
      if (currentTypingIndex < messages.length) {
        ...
        if (sequences.has(currentMsg.id) && !config.fastMode) {
          ...
          if (typingElapsed > sequence.totalDuration + messageInterval) {
            currentTypingIndex++;
          }
        } else {
      ...
      const visibleForRender = visibleMessages.slice();
